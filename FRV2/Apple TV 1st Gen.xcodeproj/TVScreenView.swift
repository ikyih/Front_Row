...
struct TVScreenView: View {
    @StateObject private var library = TVLibraryManager()
    ...
    var body: some View {
        ...
        .task {
            ...
            if library.authorizationStatus == .authorized && library.shows.isEmpty {
    ...
    private var content: some View {
        Group {
            ...
            case .denied, .restricted:
                statusText("Access denied. Enable TV Shows access in Settings.")
            case .authorized:
                if library.isLoading {
                    statusText("Loading TV Shows…")
                } else if library.shows.isEmpty {
                    statusText("No local TV episodes found.")
                } else {
                    showsList
                }
            @unknown default:
    ...
    private var showsList: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 0) {
                ForEach(library.shows, id: \.persistentID) { item in
                    TVRow(item: item) {
                        selectedItem = item
                    }
...
private struct TVRow: View {
    ...
    private var subtitle: String {
        let show = item.albumTitle ?? "TV Show"
